...
function component_modal(string $id, array $options = []): string
{
    ...
    $html .= "<div class=\"bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-gray-200 dark:border-slate-800 w-full {$maxW} max-h-[85vh] flex flex-col transform scale-95 opacity-0 transition-all duration-200\" id=\"{$id}-panel\">";

    if ($title) {
        $html .= '<div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-800">';
        $html .= '<h3 class="text-lg font-semibold text-gray-800 dark:text-white">' . htmlspecialchars($title) . '</h3>';
        $html .= '<button onclick="closeModal(\'' . $id . '\')" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors">';
        $html .= '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>';
        $html .= '</button></div>';
    }
    ...
    if ($footer) {
        $html .= "<div class=\"px-6 py-4 border-t border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-950 flex items-center justify-end gap-3\">{$footer}</div>";
    }
