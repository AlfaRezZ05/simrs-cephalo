<?php
/**
 * SIMRS-TB — Shared Sidebar Navigation (Unified Dark Design)
 */
...
<!-- SIMRS-TB Sidebar (Unified Dark Design) -->
<aside id="tb-sidebar" class="fixed left-0 top-16 w-64 h-[calc(100vh-4rem)] z-40 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 flex flex-col border-r border-white/[0.06]" style="background: rgba(2, 6, 23, 0.85); backdrop-filter: blur(24px);">
    ...
    <div class="px-5 py-5 border-b border-white/[0.06]">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-white font-bold text-sm tracking-wide">Poli Paru</h2>
                <p class="text-slate-500 text-[10px] font-semibold tracking-wider uppercase">KLINIK TB INTEGRASI</p>
            </div>
        </div>
    </div>
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto scrollbar-thin">
        <p class="px-3 mb-2 text-[10px] font-semibold text-slate-600 uppercase tracking-widest">Alur Medis TB</p>
        <?php foreach ($sidebarItems as $key => $item): 
            $isActive = ($activePage === $key);
            $activeClass = $isActive 
                ? 'bg-emerald-500/10 text-emerald-400 border-l-2 border-emerald-400' 
                : 'text-slate-500 hover:bg-white/[0.04] hover:text-slate-300 border-l-2 border-transparent';
        ?>
        <a href="<?= $item['href'] ?>" 
           class="flex items-center gap-3 px-3 py-2.5 rounded-r-lg text-sm font-medium transition-all duration-200 group <?= $activeClass ?>">
            <svg class="w-5 h-5 shrink-0 <?= $isActive ? 'text-emerald-400' : 'text-slate-600 group-hover:text-slate-400' ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <?= $item['icon'] ?>
            </svg>
    ...
    <div class="px-4 py-4 border-t border-white/[0.06] space-y-3">
        <div class="rounded-xl p-3 border border-white/[0.05]" style="background: rgba(15, 23, 42, 0.5);">
            <div class="flex items-center gap-2 mb-1.5">
                <div class="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></div>
                <span class="text-xs font-semibold text-emerald-400">Poli Terhubung</span>
            </div>
            <p class="text-[10px] text-slate-600">SIMRS-TB &bull; MedWeb AI</p>
        </div>
    </div>
</aside>

<!-- Sidebar Overlay (mobile) -->
<div id="tb-sidebar-overlay" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-30 hidden lg:hidden" onclick="toggleTbSidebar()"></div>

<!-- Mobile Sidebar Toggle -->
<button onclick="toggleTbSidebar()" 
        class="fixed bottom-5 left-5 z-50 lg:hidden w-12 h-12 bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-full shadow-lg shadow-emerald-500/25 flex items-center justify-center hover:scale-110 active:scale-95 transition-transform">
